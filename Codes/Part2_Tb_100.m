...
writematrix(XPNRZ, "PNRZ(100).txt");
writematrix(t, "TPNRZ(100).txt");
...
writematrix(XUNRZ, "UNRZ(100).txt");
writematrix(t, "TUNRZ(100).txt");
    ...
    for j = 0: (N-1)
        if X(j+1) == 1
            if (((j+0.5)*n)+d(i) <= length(t))
                ...
                if (((j+1)*n)+d(i)+1 <= length(t))
                    XMAN(i, ((j+0.5)*n)+d(i)+1:((j+1)*n)+d(i)) = -A;
                else
                    XMAN(i, ((j+0.5)*n)+d(i)+1:length(t)) = -A; 
                end    
            else
...
writematrix(XMAN, "MAN(100).txt");
writematrix(t, "TMAN(100).txt");
...
figure("Name","Ensemble ACF at Tb = 100 sec");
subplot(3, 1, 1); plot(taw1, Double_ACF1); xlabel('Taw(sec)');ylabel('Ensemble ACF'); xlim([-500 500]); 
title('Ensemble ACF of PNRZ Process at Tb = 100 sec'); grid on; 

subplot(3, 1, 2); plot(taw1, Double_ACF2); xlabel('Taw(sec)');ylabel('Ensemble ACF'); xlim([-500 500]); 
title('Ensemble ACF of UNRZ Process at Tb = 100 sec'); grid on; 

subplot(3, 1, 3); plot(taw1, Double_ACF3); xlabel('Taw(sec)');ylabel('Ensemble ACF'); xlim([-500 500]); 
title('Ensemble ACF of Manchester Process at Tb = 100 sec'); grid on; 
...
figure("Name","Power Spectral Density of the Process at Tb = 100");
subplot(3, 1, 1); plot(f, abs(PSD1)); xlabel('Frequency(Hz)');ylabel('Amplitude');  
title('PSD of PNRZ Process at Tb = 100 sec'); grid on; 

subplot(3, 1, 2); plot(f, abs(PSD2)); xlabel('Frequency(Hz)');ylabel('Amplitude'); 
title('PSD of UNRZ Process at Tb = 100 sec'); grid on; 

subplot(3, 1, 3); plot(f, abs(PSD3)); xlabel('Frequency(Hz)');ylabel('Amplitude'); 
title('PSD of Manchester Process at Tb = 100 sec'); grid on; 
