...
writematrix(XPNRZ, "PNRZ(10).txt");
writematrix(t, "TPNRZ(10).txt");
...
writematrix(XUNRZ, "UNRZ(10).txt");
writematrix(t, "TUNRZ(10).txt");
...
writematrix(XMAN, "MAN(10).txt");
writematrix(t, "TMAN(10).txt");
...
figure("Name","Ensemble ACF at Tb = 10 sec");
subplot(3, 1, 1); plot(taw1, Double_ACF1); xlabel('Taw(sec)');ylabel('Ensemble ACF'); xlim([-50 50]); 
title('Ensemble ACF of PNRZ Process at Tb = 10 sec'); grid on; 

subplot(3, 1, 2); plot(taw1, Double_ACF2); xlabel('Taw(sec)');ylabel('Ensemble ACF'); xlim([-50 50]); 
title('Ensemble ACF of UNRZ Process at Tb = 10 sec'); grid on; 

subplot(3, 1, 3); plot(taw1, Double_ACF3); xlabel('Taw(sec)');ylabel('Ensemble ACF'); xlim([-50 50]); 
title('Ensemble ACF of Manchester Process at Tb = 10 sec'); grid on; 
...
figure("Name","Power Spectral Density of the Process at Tb = 10");
subplot(3, 1, 1); plot(f, abs(PSD1)); xlabel('Frequency(Hz)');ylabel('Amplitude');  
title('PSD of PNRZ Process at Tb = 10 sec'); grid on; 

subplot(3, 1, 2); plot(f, abs(PSD2)); xlabel('Frequency(Hz)');ylabel('Amplitude'); 
title('PSD of UNRZ Process at Tb = 10 sec'); grid on; 

subplot(3, 1, 3); plot(f, abs(PSD3)); xlabel('Frequency(Hz)');ylabel('Amplitude'); 
title('PSD of Manchester Process at Tb = 10 sec'); grid on; 
